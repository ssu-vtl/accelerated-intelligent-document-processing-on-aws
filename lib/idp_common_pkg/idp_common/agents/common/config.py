...
def get_environment_config(required_keys: Optional[list] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning(f"Invalid STRANDS_LOG_LEVEL: {strands_log_level_str}, defaulting to INFO")
...
def configure_logging(log_level=None, strands_log_level=None):
    ...
    logger.debug(f"Configured application logging level: {logging.getLevelName(log_level)}")
    logger.debug(f"Configured Strands framework logging level: {logging.getLevelName(strands_log_level)}")

...
class SummaryMarkdownFormatter:
    ...
    def __init__(self, document, section_summaries, is_section=False, include_toc=True):
        ...
        self.section_order = []
        self.is_section = is_section  # Flag for section vs. combined document formatting
        ...
        for section_id in self.section_order:
            if section_id in self.raw_section_summaries:
                ...
                if isinstance(section_data, dict) and "content" in section_data:
                    ...
                    section_name = section_data.get("title") or self._get_section_name(section_id)
                else:
                ...
                section_content = self._remove_existing_title(section_content, section_name)
                
                # Process citations
                section_content = self.process_citations(section_content, formatted_name)
    ...
    def _remove_existing_title(self, content: Any, section_name: str) -> Any:
        ...
        if not isinstance(content, str):
            ...
            if isinstance(content, dict) and "summary" in content and isinstance(content["summary"], str):
                content["summary"] = self._remove_existing_title(content["summary"], section_name)
            return content
    ...
    def process_citations(self, content: Any, section_name: str) -> Any:
        ...
        if not isinstance(content, str):
            if isinstance(content, dict) and "summary" in content and isinstance(content["summary"], str):
                content["summary"] = self.process_citations(content["summary"], section_name)
            return content
        ...
        for i, section in enumerate(sections):
            if section.strip().startswith('References') or section.strip().startswith('[Cite-'):
                ...
                new_anchor = f'<a id="{clean_section_name.lower()}-cite-\\1-page-\\2"></a>'
    ...
    def add_navigation_aids(self):
        ...
        ordered_sections = [s for s in self.section_order if s in self.formatted_sections]
    ...
    def _combine_markdown(self, toc: str) -> str:
        ...
        for i, section_id in enumerate(ordered_sections, 1):
            ...
            md_parts.append(f"## {i}. {name} <a id=\"{anchor}\"></a>\n\n{section_content}")
            md_parts.append("\n\n")

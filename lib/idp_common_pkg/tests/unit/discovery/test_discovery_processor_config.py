...
class TestDiscoveryProcessorConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_config = {
            "discovery": {
                ...
                "output_format": {"sample_json": '{"test": "format"}'},
            }
        }
    ...
    def test_load_discovery_configuration_from_dynamodb(self, mock_boto3_resource):
        ...
        mock_table.get_item.return_value = {"Item": self.sample_config}
        ...
        def mock_load_discovery_configuration():
            try:
                configuration_table_name = os.environ.get(
    ...
    @patch("boto3.resource")
    def test_load_discovery_configuration_fallback_to_default(
    ...
    def test_get_default_discovery_config_uses_env_vars(self):
        ...
        self.assertEqual(
            config["discovery"]["without_ground_truth"]["model_id"], "env-model-id"
        )
        self.assertEqual(
            config["discovery"]["with_ground_truth"]["model_id"], "env-model-id"
        )
    ...
    def test_process_discovery_job_uses_config(self, mock_classes_discovery):
        ...
        mock_instance.discovery_classes_with_document.return_value = {
            "status": "SUCCESS"
        }
        ...
        mock_instance.discovery_classes_with_document.assert_called_once_with(
            input_bucket="test-bucket", input_prefix="test-doc.pdf"
        )
    ...
    def test_process_discovery_job_with_ground_truth(self, mock_classes_discovery):
        ...
        mock_instance = Mock()
        mock_classes_discovery.return_value = mock_instance
        mock_instance.discovery_classes_with_document_and_ground_truth.return_value = {
            "status": "SUCCESS"
        }
    ...
    def test_default_config_structure_completeness(self):
        ...
        for field in required_fields:
            self.assertIn(
                field, without_gt, f"Missing field: {field} in without_ground_truth"
            )
        ...
        for field in required_fields:
            self.assertIn(
                field, with_gt, f"Missing field: {field} in with_ground_truth"
            )

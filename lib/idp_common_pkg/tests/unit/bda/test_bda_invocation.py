...
def test_get_custom_output_no_metadata():
    ...
    with pytest.raises(ValueError, match="Job metadata not populated. Use from_s3\\(\\) first."):
...
def test_get_custom_output_no_segment_metadata():
    ...
    with pytest.raises(ValueError, match="No segment metadata found for segment index 0"):
        invocation.get_custom_output()
...
def test_get_custom_output_no_custom_output_path():
    ...
    with pytest.raises(ValueError, match="No custom output path found for asset ID 0, segment index 0"):
        invocation.get_custom_output()
...
def test_get_custom_output_invalid_s3_uri():
    ...
    invocation = BdaInvocation(
        job_metadata={
            "job_id": "job-123", 
            "output_metadata": [{"segment_metadata": [{"custom_output_path": "invalid-path"}]}]
        }
    )
...
def test_get_custom_output_success(mock_s3_util):
    ...
    invocation = BdaInvocation(
        job_metadata={
            "job_id": "job-123", 
            "output_metadata": [{"segment_metadata": [{"custom_output_path": custom_output_path}]}]
        }
    )

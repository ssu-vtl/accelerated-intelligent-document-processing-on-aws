...
import onShareDocuments from '../graphql/queries/onShareDocuments';
...
const useCallsGraphQlApi = ({ initialPeriodsToLoad = CALL_LIST_SHARDS_PER_DAY * 2 } = {}) => {
  ...
  useEffect(() => {
    logger.debug('onShareDocuments subscription');
    const subscription = API.graphql(graphqlOperation(onShareDocuments)).subscribe({
      next: async ({ provider, value }) => {
        logger.debug('share meetings subscription update', { provider, value });
        const sharedCalls = value?.data?.onShareDocuments.Calls || '';

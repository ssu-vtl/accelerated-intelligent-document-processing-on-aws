...
export const COLUMN_DEFINITIONS_MAIN = [
  {
    id: KEY_COLUMN_ID,
    header: 'Document ID',
    cell: (item) => <Link href={`#${DOCUMENTS_PATH}/${item.callId}`}>{item.callId}</Link>,
    sortingField: 'callId',
    width: 325,
  },
  {
...
const VISIBLE_CONTENT_OPTIONS = [
  {
    ...
    options: [
      { id: 'callId', label: 'Document ID', editable: false },
      { id: 'agentId', label: 'Name' },
      { id: 'owner', label: 'Owner' },
      { id: 'sharedWith', label: 'Shared With' },
      { id: 'initiationTimeStamp', label: 'Completion Timestamp' },
...
export const CallsCommonHeader = ({ resourceName = 'Documents', ...props }) => {

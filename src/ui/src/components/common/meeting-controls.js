...
import shareDocuments from '../../graphql/queries/shareDocuments';
import deleteDocuments from '../../graphql/queries/deleteDocuments';
...
const invokeShareDocuments = async (props, currentRecipients) => {
  const callsKeys = callsWithKeys(props);
  const response = await API.graphql({
    query: shareDocuments,
    variables: {
      input: { Calls: callsKeys, MeetingRecipients: currentRecipients },
    },
  });

  const result = response.data.shareDocuments.Result;
  return result;
};

const invokeDeleteDocuments = async (props) => {
  const callsKeys = callsWithKeys(props);
  const response = await API.graphql({
  ...
  const result = response.data.deleteDocuments.Result;
  return result;
};
...
export const shareModal = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await invokeShareDocuments(props, allRecipients.join(','));
    setCurrentRecipients([]);
    setNewRecipients([]);
    setSubmit(false);

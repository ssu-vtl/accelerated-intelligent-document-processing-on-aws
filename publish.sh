...
echo CLI Deploy: aws cloudformation deploy --region $REGION --template-file .aws-sam/${MAIN_TEMPLATE} --s3-bucket ${BUCKET} --s3-prefix ${PREFIX_AND_VERSION} --capabilities CAPABILITY_NAMED_IAM CAPABILITY_AUTO_EXPAND --parameter-overrides UDOPModelArtifactPath="s3://bucket-name/path/to/model.tar.gz" --stack-name "<your-stack-name>"
echo Done
exit 0
